...
for pictures in $list; do 
	...
	for i in flip_n.png flip_v.png; do
		for angle in ${angles[@]}; do 
			echo "${cur_picture}_rot_${angle}_${i} ${cur_cat}" >> augmented_${name}_temp.txt
		done
	done
done

cat augmented_${name}_temp.txt | sort >> augmented_${name}
rm augmented_${name}_temp.txt
rm temp.txt
