...
files=($(set -f;find . -maxdepth 1 -type f -regex ./${1}_tenfold_[A-J]_.*.txt | cut -d / -f 2 | sort))
...
if test "${#files[@]}" -ne 20; then
	echo "Tenfold missing files? Found:"
	echo "${files[@]}"
	exit 1
fi

for file_idx in {0..19..2}; do
	(
		echo '('
