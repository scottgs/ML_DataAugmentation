...
# creates configs for tenfold cross validation
...
# $2_tenfold_$LABEL_test.txt
# $2_tenfold_$LABEL_train.txt
...
for label_idx in "${!labels[@]}"; do
	...
	test_fname=${base_dir}/${2}_tenfold_${labels[$label_idx]}_test.txt
	train_fname=${base_dir}/${2}_tenfold_${labels[$label_idx]}_train.txt

	for cat_num in "${!cat_list[@]}"; do
		# selected are the files whose 2 digit id number ends in the selected label_idx number (0-9)
		selected_test=($(set -f;find ${cat_list[$cat_num]} -type f -regex ${cat_list[$cat_num]}/${cat_list[$cat_num]}[0-9][${label_idx}].*))
		# train is the inverse
		selected_train=($(set -f;find ${cat_list[$cat_num]} -type f -regex ${cat_list[$cat_num]}/${cat_list[$cat_num]}[0-9][^${label_idx}].*))
