...
for file in "${source_files[@]}"; do
	...
	for angle in "${angles[@]}"; do
		(
			new_fname="${file%.tif}_rot_${angle}"
			convert ${file} -rotate ${angle} +repage -gravity center -crop 227x227+0+0 +repage -write ${new_fname}_flip_n.tif \
				-flip +repage ${new_fname%.tif}_flip_v.tif

			# convert ${new_fname} -flop +repage ${new_fname%.tif}_flip_h.tif
		) &
	done
	wait
	rm $file
